const DEFAULT_API_BASE = "https://prophet-pushing-retirement-tension.trycloudflare.com";
const API_BASE = (
  window.API_BASE ||
...
btn.onclick = async () => {
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(data?.detail || data?.message || "Gagal memeriksa URL");
    }
    ...
    const vtStatus = data?.vt?.status || "unknown";
    ...
    const maliciousPct = vtTotal ? Math.round((vt.malicious / vtTotal) * 100) : 0;
    const suspiciousPct = vtTotal ? Math.round((vt.suspicious / vtTotal) * 100) : 0;
    const harmlessPct = vtTotal ? Math.round((vt.harmless / vtTotal) * 100) : 0;

    const vtStatusText = {
      completed: "Selesai",
      pending: "Sedang diproses",
      unavailable: "Tidak tersedia",
      trusted_domain: "Domain tepercaya",
...
${verdict.text}.\n\n${reasons.length ? reasons.map(r => `• ${r}`).join("\n") : "Tidak ada sinyal khusus yang kuat."}
        ...
        <div class="card">
          ...
          <div class="pretty">
            Jangan asal klik link mencurigakan.<br><br>
            Gunakan hasil scan ini sebagai referensi, bukan keputusan mutlak.<br><br>
